...
class HeadCountPage extends Component {
    ...
    renderMealHeadCountInfo = () => {
        ...
        return (
            ...
            <HeadCountWrapper>
              ...
              <DateWrapper>
                  <SetDate onChangeDate={onChangeDate}
    ...
    renderSummaryInfo = () => {
        ...
        return (
            ...
            <TotalHeadCount>{selectedMealTypeheadCount.summary.total_meal_head_count}</TotalHeadCount>
            <CompletedMealHeadCount>{selectedMealTypeheadCount.summary.completed_meal_head_count}</CompletedMealHeadCount>
            </React.Fragment>
    ...
    renderPreferenceBaseInfo = () => {
        ...
        <Count>{selectedMealTypeheadCount.head_count.full_meal_head_count}</Count>
        </MealInfo>
        <MealInfo>
        <Meal>{strings.headCountpage.halfMealHeadCount}</Meal>
        <Count>{selectedMealTypeheadCount.head_count.half_meal_head_count}</Count>
        </MealInfo>
        <MealInfo>
        <Meal>{strings.headCountpage.customMealHeadCount}</Meal>
        <Count>{selectedMealTypeheadCount.head_count.custom_meal_head_count}</Count>
        </MealInfo>
        <MealInfo>
        <Meal>{strings.headCountpage.skipMealHeadCount}</Meal>
        <Count>{selectedMealTypeheadCount.head_count.skipped_meal_head_count}</Count>
        </MealInfo>
        </React.Fragment>)
    }

    render() {
        const { headCountAPIError, headCountAPIStatus, onRetry, gotoHome, onClickSignOut } = this.props
        return (
            <LoadingWrapper>
            <HeaderWrapper>
